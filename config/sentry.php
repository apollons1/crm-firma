...
return [
    ...
    // Filtrează ce ajunge efectiv în Sentry: nimic în local (APP_ENV=local),
    // și nici 404-uri / erori de validare — acelea sunt UX normal, nu erori
    // reale de urmărit. Se aplică oricărui eveniment (excepții necapturate
    // ȘI log-uri trimise prin canalul "sentry", vezi config/logging.php).
    // Callable ca array de string-uri (nu Closure) — obligatoriu pentru ca
    // fișierul să rămână compatibil cu `artisan config:cache`.
    'before_send' => [SentryEventFilter::class, 'beforeSend'],
